...
Русский:Для работы библиотеки необходимо в папку проекта(рядом с exe), в т.ч. при сборке положить файлы:libiconv-2.dll, libintl-9.dll, libpq.dll, libxml2.dll, libxslt.dll,libwinpthread-1.dll а также для последних версий PostgreSQL libssl-X-x64.dll и libcrypto-x-x64.dll из папки PostgreSQL\<version>\bin. Значение x в библиотеках зависит от версии PostgreSQL. Если терминал пишет, что недоступны драйвера, проверте папку sqldrivers.
